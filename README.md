...
- `src/ingestion/`: đọc PDF, tạo JSON có cấu trúc, chia chunk và build vector database.
...
- `data/processed/`: JSON có cấu trúc sinh tự động từ RAW PDF, không commit.
...
Không commit `.env`, `.venv/`, `data/processed/` hoặc `chroma_db/`; thành viên sẽ tự tạo các thư mục generated này theo hướng dẫn ở trên.
